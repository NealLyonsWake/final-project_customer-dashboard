...
def handle_branch_performance(top_or_bot, region_selected, overall_branch_performance):
    ...
    if top_or_bot == "top":
        ...
        figure = ex.bar(per_region, x='branch_name', y='performance', color='branch_name', title=f'Top branches in {region_selected}')
        return figure
    else:
        per_region = overall_branch_performance[overall_branch_performance['region']==region_selected].tail(10)
        figure = ex.bar(per_region, x='branch_name', y='performance', color='branch_name', title=f'Bottom branches in {region_selected}')
        return figure